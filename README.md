...
This is the standalone backend extracted from the monorepo `AI-for-Diabetic-Retinopathy-Screening-in-Rural-India`. It serves the EfficientNetB0 5-grade DR classifier (``Aldahmashi/DR-EfficientNetB0`` on Hugging Face Hub) through the FULL pipeline - quality gate, preprocessing, classification, referable-DR check, lesion/vessel analysis (OpenCV heuristics), Grad-CAM heatmap + enhanced scan, clinical report - over ``POST /api/screen`` and ``POST /api/screen-stream`` (SSE), plus ``GET /api/health`` and ``GET /api/models/status'.
...
2. **Free-tier fit.** Render Free gives 512 MB RAM and a small disk. The full monorepo backend (JAX + torch + Keras + lesion detectors + vessel analysis + Grad-CAM + scipy stack) cannot fit. So this repo ships the full pipeline with the SAME lean footprint: the lesion/vessel detectors are pure OpenCV heuristics (no extra weights), Grad-CAM reuses the loaded classifier, and the report generator is pure Python - so CPU-only torch + keras + opencv-headless is all it needs.
